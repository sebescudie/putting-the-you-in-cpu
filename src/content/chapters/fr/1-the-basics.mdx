...
Intel et AMD se sont débrouillés pour ne pas bien s'accorder sur le x86-64; il contient en fait deux groupes d'instructions d'appels système. [SYSCALL](https://www.felixcloutier.com/x86/syscall.html) et [SYSENTER](https://www.felixcloutier.com/x86/sysenter) sont des alternatives optimisées à des instructions comme `INT 0x80`. Leurs instructions de retour respectives, [SYSRET](https://www.felixcloutier.com/x86/sysret.html) et [SYSEXIT](https://www.felixcloutier.com/x86/sysexit), permettent de revenir rapidement en user space et de revenir au code du programme.

(Les processeurs AMD et Intel ont des compatibilités légèrement variante avec ces instructions. `SYSCALL` est souvent la meilleure option pour des programmes 64 bits, alors que `SYSENTER` est mieux supporté dans les programmes 32 bits)

Dans leur propre style, les architectures RISC(https://fr.wikipedia.org/wiki/Processeur_à_jeu_d'instructions_réduit) n'ont pas ce genre d'instructions. AArch64, l'architecture RISC sur laquelle est basée Apple Silican, n'utilise [qu'une seule instruction d'interruption](https://developer.arm.com/documentation/ddi0596/2021-12/Base-Instructions/SVC--Supervisor-Call-) pour les appels système et les interruptions. Je pense que les utilisateurs Mac le vivent bien :)

---

Wow, ça fait un gros morceau! Faisons un petit récap :

- Les processeurs exécutent des instructions dans une boucle infinie de cycles d'éxecutions (fetc-execute loop), et n'ont aucune idée de ce qu'est un système d'exploitation ou un programme. Le mode du processeur, généralement stocké dans un registre, determine quelles instructions peuvent être exécutées. Le code du système d'exploitation s'exécute en mode kernel et bascule en mode utilisateur pour exécuter des programmes.
- Pour exécuter une application, le système d'exploitation bascule en mode utilisateur et pointe le processeur vers le point d'entrée du code dans la RAM. Parce qu'ils n'ont que les droits offerts par le mode utilisateur, les programmes qui veulent intéragir avec l'extérieur doivent demander de l'aide à du code du système d'exploitation. Les appels système constituent une façon standard pour les programmes de basculer du mode utilisateur vers le mode kernel et donc vers du code de l'OS
- Les programmes utilisent ces appels système via des fonctions de librairies partagées. Elles enveloppent du code machine pour soit une interruption logicielle ou un appel système spécifique à une architecture qui délègue le contrôle au kernel de l'OS et change le niveau de sécurité. Le kernel fait son taff et bascule en mode utilisateur, retournant au code du programme.

Essayons de répondre à ma première question :

> Si le CPU n'est pas capable de suivre plusieurs processus et se contente d'exécuter des instructions les unes après les autres, pourquoi il ne reste pas bloqué dans un programme ? Comment fait-il pour faire tourner plusieurs programmes à a fois  ?

La réponse à cette question, cher ami, est aussi la réponse à pourquoi Coldplay est si populaire... Clocks, les horloges! (En vrai on appelle ça des timers. Je voulais juste glisser cette vanne).